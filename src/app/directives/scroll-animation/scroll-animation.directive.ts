...
export class ScrollAnimationDirective implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit(): void {
        ...
        this.resizeUnlistenerFn = this.renderer.listen(window, 'resize', this.onResize.bind(this));
    }
    
    private onScroll(): void {
        if (!this.isAnimating) {
            if (window.pageYOffset > this.elementPosition - (window.innerHeight / 2) + this.scrollDelay) {
                if (!this.isInAnimationDone && this.inAnimationData) {
                    const inAnimation = this.animationService.buildAnimation(this.element, this.inAnimationData);
                ...
                if (this.isInAnimationDone && this.outAnimationData) {
                    const outAnimation = this.animationService.buildAnimation(this.element, this.outAnimationData);
                    this.isAnimating = true;
